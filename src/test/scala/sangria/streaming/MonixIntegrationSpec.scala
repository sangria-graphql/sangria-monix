...
class MonixIntegrationSpec extends WordSpec with Matchers {
  ...
  "Monix Integration" should {
    ...
    "mapFuture" in {
      res(impl.mapFuture(Observable(1, 2, 10))(x ⇒ Future.successful(x + 1))) should be (List(2, 3, 11))
    }
    ...
    "flatMapFuture" in {
      res(impl.flatMapFuture(Future.successful(1))(i ⇒ Observable(i.toString, (i + 1).toString))) should be (List("1", "2"))
    }
